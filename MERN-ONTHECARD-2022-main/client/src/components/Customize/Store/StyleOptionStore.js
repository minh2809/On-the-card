...
import { setBGColorStyleStore } from "../../../store/actionCreators";
import { getCorrectColor } from "../../../utilities/helper_functions";

const StyleOptionStore = ({ Vietnamese, English }) => {
  const { appLanguage, storePage } = useSelector((state) => state);
  const { backgroundColorObject, backgroundColor } = storePage;
  ...
  useEffect(() => {
    ...
    if (currentBgColor && currentBgStyle) {
      dispatch(setBGColorStyleStore(currentBgColor, currentBgStyle));
      setSelectedColorStyle(currentBgStyle);
    } else {
      dispatch(setBGColorStyleStore(bgColor, bgStyle));
  ...
  const handleSelectColorStyle = (colorStyle) => () => {
    ...
    dispatch(setBGColorStyleStore(correctColor, colorStyle));
  };
...
export default StyleOptionStore;
