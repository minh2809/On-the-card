...
import { setBackgroundColorStyle } from "../../store/actionCreators";
import { getCorrectColor } from "../../utilities/helper_functions";

const StyleOptions = ({ Vietnamese, English }) => {
  const { appLanguage, userInfo } = useSelector((state) => state);
  const { backgroundColorObject, backgroundColor } = userInfo;
  ...
  useEffect(() => {
    ...
    if (currentBgColor && currentBgStyle) {
      dispatch(setBackgroundColorStyle(currentBgColor, currentBgStyle));
      setSelectedColorStyle(currentBgStyle);
    } else {
      dispatch(setBackgroundColorStyle(bgColor, bgStyle));
  ...
  const handleSelectColorStyle = (colorStyle) => () => {
    ...
    dispatch(setBackgroundColorStyle(correctColor, colorStyle));
  };
