...
const StoreImage = ({ uploadAvatar }) => {
  const { userInfo, storePage } = useSelector((state) => state);
  const { email: userEmail } = userInfo;
  const { avatarImg: avatar, avatarURL } = storePage;
  ...
  const updateImage = (imagePassed, imageUrl) => {
    dispatch({
      type: actionTypes.IMAGEUPDATESTORE,
      ...
      <div className={classes.formInput}>
        ...
        {uploadAvatar ? (
          <>
            <label
              htmlFor="file-ip-2"
              className={classes.imageUploadLabelProduct}
            >
