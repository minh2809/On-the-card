...
import { setIconEnterprise } from "../../../store/actionCreators";

const IconEnterprise = ({ Vietnamese, English }) => {
  const { appLanguage, enterprisePage } = useSelector((state) => state);
  const { iconStyle } = enterprisePage;
  ...
  const handleSelectColorStyle = (iconStyle) => () => {
    ...
    dispatch(setIconEnterprise(iconStyle || "Original"));
  };
